...
describe('AuditService', () => {
  ...
  const row = (over: Record<string, unknown> = {}) => ({
    id: 'e1', actorId: 'admin-1', action: 'staff.create', entity: 'User',
    entityId: 'user-9', meta: { role: 'ADMIN' }, createdAt: new Date('2026-07-09T10:00:00.000Z'),
    ...over,
  });
  const actor = (id: string, roles: string[]) => ({
    id, username: `name-${id}`, email: `${id}@acme.test`,
  ...
  it('record persists via the repository', async () => {
    await svc.record({ actorId: 'admin-1', action: 'staff.create', entity: 'User', entityId: 'user-9', meta: { role: 'ADMIN' } });
    expect(repo.create).toHaveBeenCalledWith({
      actorId: 'admin-1', action: 'staff.create', entity: 'User', entityId: 'user-9', meta: { role: 'ADMIN' },
    });
  });
  ...
  it('list resolves the actor into username/email/roles', async () => {
    ...
    expect(res.items[0].actor).toEqual({
      id: 'admin-1', username: 'name-admin-1', email: 'admin-1@acme.test', roles: ['SUPER_ADMIN'],
    });
  });
  ...
  it('builds inclusive date bounds (date-only "to" covers the whole day)', async () => {
    await svc.list({ page: 1, limit: 20, order: 'desc', from: '2026-07-01', to: '2026-07-09' } as any);
  ...
  it('exportCsv emits a header row and one line per event', async () => {
    repo.findMany.mockResolvedValue([row({ actorId: null, action: 'files.delete', entity: 'FileAsset', entityId: 'asset-3' })]);
    ...
    expect(lines[1]).toBe('2026-07-09T10:00:00.000Z,System,,,files.delete,FileAsset,asset-3');
  });
  ...
  it('exportCsv resolves actor columns and RFC-4180-escapes commas/quotes', async () => {
    ...
    repo.findActorsByIds.mockResolvedValue([
      { id: 'admin-1', username: 'Doe, Jane', email: 'jane@acme.test', roles: [{ role: 'SUPER_ADMIN' }, { role: 'ADMIN' }] },
    ]);

    const csv = await svc.exportCsv({ order: 'desc' } as any);
    const line = csv.split('\r\n')[1];

    expect(line).toBe('2026-07-09T10:00:00.000Z,"Doe, Jane",jane@acme.test,SUPER_ADMIN ADMIN,item.update,User,user-9');
  });

  it('caps the CSV at EXPORT_CAP rows and warns only when more exist', async () => {
    const many = Array.from({ length: 5001 }, (_, i) => row({ id: `e${i}`, actorId: null }));
  ...
  it('RFC-4180-escapes quotes and newlines in a CSV field', async () => {
    repo.findMany.mockResolvedValue([row({ actorId: null, entity: 'Note "A"', entityId: 'x\ny' })]);
