...
const config = { apiKey: 'key-123', fromEmail: 'no-reply@acme.example', fromName: 'Acme Inc' };
...
describe('BrevoMailTransport', () => {
  it('POSTs the mapped payload to Brevo with the api-key header', async () => {
    ...
    await transport.send({ to: 'jan@x.com', subject: 'Hi', html: '<p>h</p>', text: 't' });
    ...
    expect(body.sender).toEqual({ email: config.fromEmail, name: config.fromName });
  ...
  it('throws when Brevo returns a non-2xx status', async () => {
    jest.spyOn(globalThis, 'fetch').mockResolvedValue(new Response('bad', { status: 400 }));
    const transport = new BrevoMailTransport(config);
    await expect(
