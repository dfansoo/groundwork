...
describe("uploadImage", () => {
  ...
  it("rejects an unsupported type before calling the network", async () => {
    ...
    const gif = new File([new Blob([new Uint8Array(10)], { type: "image/gif" })], "a.gif", { type: "image/gif" });
  ...
  it("runs init -> PUT -> confirm and returns { fileId, url }", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith("/api/bff/admin/files/uploads")) {
        return new Response(JSON.stringify({ data: { assetId: "asset1", uploadUrl: "https://storage/put/asset1" } }), {
      ...
      if (url.endsWith("/api/bff/admin/files/asset1/confirm")) {
        return new Response(JSON.stringify({ data: { id: "asset1", url: "https://cdn/asset1.jpg" } }), {
          status: 201, headers: { "content-type": "application/json" },
        });
      }
