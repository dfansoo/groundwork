...
export interface BackendTokens {
  user: { id: string; name: string; email: string; avatar: string | null; roles: Role[] };
  accessToken: string;
  refreshToken: string;
...
function normalize(r: AuthResponse): BackendTokens {
  return {
    user: {
      ...
      // The admin console gates its nav on these. The backend still enforces
      // every call, so this is UX only.
      roles: (r.user.roles ?? []).map((r) => r.role),
    },
